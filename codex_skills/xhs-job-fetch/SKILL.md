...
- 不要求用户手动先跑 `setup`；`all` 会自动检查并触发初始化。
...
1. 先用统一入口安装：`scripts/xhs_skill.sh setup`。
...
入口动作：
- `setup`：安装初始化（调用 `scripts/install.sh`）
- `clarify`：仅打印需求澄清模板
- `login`：仅执行登录检查（可加 `--with-qr`）
- `run`：仅执行抓取
- `all`：先登录检查再抓取（推荐）
- `login` / `run` / `all` 会自动检查环境，未初始化时自动触发 `setup`
- 默认从 `https://github.com/xpzouying/xiaohongshu-mcp` 安装 MCP（也可用 `--mcp-repo` 覆盖）
